/**
 * ImportaCartellaV2Modal — versione v2 di ImportaCartellaModal.
 * Stessa logica in 4 fasi ma usa entità v2: immobile e persona.
 */
import { useState, useRef } from "react";
import { archivioApi }     from "../../api.js";
    ...
    <span style={{
      ...
      {onRemove && (
        <button onClick={onRemove} style={{ background: "none", border: "none", cursor: "pointer",
                                            padding: 0, lineHeight: 1, color, fontSize: 12, fontWeight: 700 }}>×</button>
      )}
    </span>
  );
}
...
function RigaFile({ riga, idx, tipi, entitaOptions, onUpdate }) {
  ...
  const assocLabels = riga.associazioni.map(a => {
    ...
    return found
      ? { label: `${ENTITA_LABELS[a.entita_tipo]}: ${found.nome}`, color: ENTITA_COLORS[a.entita_tipo] || "#666", key: `${a.entita_tipo}-${a.entita_id}` }
      : null;
  }).filter(Boolean);
      ...
      <tr style={{
        ...
        <td style={{ padding: "6px 8px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
            <i className={`ti ${mimeIcon(riga.file)}`} style={{ fontSize: 16, color: "var(--text2)", flexShrink: 0 }} />
            <div>
              <div style={{ fontSize: 12, fontWeight: 600, wordBreak: "break-all" }}>{riga.file.name}</div>
        ...
        <td style={{ padding: "6px 8px", width: 140 }}>
          {tipoNome
            ? <span style={{ display: "inline-block", padding: "2px 8px", borderRadius: 10,
                              ...
                              background: "rgba(107,114,128,0.15)", color: "var(--text2)" }}>{tipoNome}</span>
        ...
        <td style={{ padding: "6px 6px", textAlign: "right", width: 80, whiteSpace: "nowrap" }}>
          {isModified && (
            <span style={{ fontSize: 9, padding: "1px 5px", borderRadius: 8,
                           background: "rgba(234,179,8,0.2)", color: "#ca8a04", marginRight: 4 }}>mod.</span>
          )}
        ...
        <tr style={{ borderBottom: "1px solid var(--border)" }}>
          ...
          <td colSpan={5} style={{ padding: "0 8px 12px 8px" }}>
            <div style={{ background: "var(--bg2)", borderRadius: 8,
                          border: "1px solid rgba(59,130,246,0.2)", padding: 12, display: "grid", gap: 12 }}>
              <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "flex-end" }}>
                <div style={{ flex: "1 1 160px" }}>
                  <label style={{ fontSize: 11, display: "block", marginBottom: 3 }}>Tipo documento</label>
                  <select value={riga.tipDocId} onChange={e => setField("tipDocId", e.target.value)}
                ...
                <div style={{ flex: "1 1 200px" }}>
                  ...
                  <input value={riga.note} onChange={e => setField("note", e.target.value)}
                         style={{ width: "100%", fontSize: 12 }}
                         placeholder="Note specifiche per questo file…" />
                </div>
                {isModified && (
                  <button onClick={() => onUpdate(idx, {
                    tipDocId: riga._baseTipDocId, note: riga._baseNote,
                    associazioni: riga._baseAssoc.map(a => ({ ...a })),
                  })} style={{ background: "none", border: "1px solid var(--border)",
                                borderRadius: 6, padding: "4px 10px", cursor: "pointer",
                                fontSize: 11, color: "var(--text2)" }}>
                    <i className="ti ti-refresh" style={{ marginRight: 4 }} />Ripristina base
                  </button>
                )}
              </div>
              <AssocEditor associazioni={riga.associazioni} entitaOptions={entitaOptions}
...
export default function ImportaCartellaV2Modal({ tipi, immobili, persone, onSaved, onClose }) {
  ...
  function buildRighe() {
    setRighe(files.map(f => ({
      file: f, includi: true,
      tipDocId: base.tipDocId, note: base.note,
      associazioni: base.associazioni.map(a => ({ ...a })),
      _baseTipDocId: base.tipDocId, _baseNote: base.note,
  ...
  const renderSelezione = () => (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div style={{ display: "flex", gap: 8, alignItems: "flex-start", padding: "10px 14px",
        ...
        <i className="ti ti-info-circle" style={{ fontSize: 18, flexShrink: 0, color: "var(--accent)", marginTop: 1 }} />
        <div>
          ...
          Potrai assegnare tag comuni e personalizzarli per ogni file.
        </div>
      </div>
      <div style={{ display: "flex", gap: 10, flexDirection: "column" }}>
        <button onClick={() => folderRef.current.click()} style={{
          ...
          <div style={{ textAlign: "left" }}>
            ...
            <div style={{ fontSize: 12, color: "var(--text2)" }}>Carica tutti i file di una cartella (incluse sottocartelle)</div>
          </div>
        </button>
        ...
        <button onClick={() => fileRef.current.click()} style={{
          ...
          <div style={{ textAlign: "left" }}>
            ...
            <div style={{ fontSize: 12, color: "var(--text2)" }}>Scegli manualmente i file da importare</div>
          </div>
        </button>
  ...
  const renderBase = () => (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <Field label="Tipo documento">
        <select value={base.tipDocId} onChange={e => setBase(b => ({ ...b, tipDocId: e.target.value }))}>
          <option value="">— nessuno —</option>
          {tipi.map(t => <option key={t.id} value={t.id}>{t.nome}</option>)}
        </select>
      </Field>
      <Field label="Note comuni" hint="Opzionale — aggiunte a tutti i documenti">
        <input value={base.note} onChange={e => setBase(b => ({ ...b, note: e.target.value }))}
               placeholder="Nota comune…" />
      </Field>
      <div>
        <label style={{ fontSize: 12, fontWeight: 600, color: "var(--text2)", display: "block", marginBottom: 10 }}>
          Associazioni comuni
        </label>
        <AssocEditor associazioni={base.associazioni} entitaOptions={entitaOptions}
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center",
          ...
          {modified > 0 && <>
            <span style={{ color: "var(--text2)" }}>·</span>
            <span style={{ color: "#ca8a04" }}><strong>{modified}</strong> con tag personalizzati</span>
          </>}
          <div style={{ flex: 1 }} />
        ...
        <div style={{ overflowY: "auto", maxHeight: "55vh", border: "1px solid var(--border)", borderRadius: 8 }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
            ...
            <tbody>
              {righe.map((r, i) => (
                <RigaFile key={i} riga={r} idx={i} tipi={tipi}
                          entitaOptions={entitaOptions} onUpdate={updateRiga} />
              ))}
            </tbody>
          </table>
        </div>
  ...
  const title  = importing ? `Caricamento… ${progress.done}/${progress.total}` : titles[fase] || "Importa da cartella";
  ...
  const footer = fase < 3 && !importing ? (
    <>
      ...
      {fase === 2 && (
        <Btn variant="primary" onClick={doImport} disabled={!righe.some(r => r.includi)}>
          <i className="ti ti-cloud-upload" /> Carica {righe.filter(r => r.includi).length} file
        </Btn>
      )}
    </>
  ) : null;
          ...
          {["Selezione", "Tag base", "Revisione"].map((s, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 6 }}>
              ...
              <span style={{ fontSize: 12, color: i === fase ? "var(--text)" : "var(--text2)",
                             fontWeight: i === fase ? 600 : 400 }}>{s}</span>
