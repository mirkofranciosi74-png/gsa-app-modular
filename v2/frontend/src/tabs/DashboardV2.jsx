...
// ─────────────────────────────────────────────────────────────────────────────
// KPI card con breakdown per appartamento
// ─────────────────────────────────────────────────────────────────────────────
function KpiCard({ label, value, icon, color, bg, rows, onClick }) {
  return (
    <div className="card" onClick={onClick}
         style={{ cursor: onClick ? "pointer" : "default", borderColor: color + "44" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
        <div style={{ width: 34, height: 34, borderRadius: 8, background: bg, flexShrink: 0,
                      display: "flex", alignItems: "center", justifyContent: "center" }}>
          <i className={`ti ${icon}`} style={{ fontSize: 17, color }} />
        </div>
        <span style={{ fontSize: 11, color: "var(--text2)", fontWeight: 500 }}>{label}</span>
      </div>
      <p style={{ fontSize: 22, fontWeight: 700, color, margin: 0, marginBottom: rows?.length ? 8 : 0 }}>
        {value}
      </p>
      {rows?.length > 0 && (
...
// ─────────────────────────────────────────────────────────────────────────────
// Tabella generica con header e righe
// ─────────────────────────────────────────────────────────────────────────────
function Tabella({ headers, rows, totRows }) {
  return (
    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
      <thead>
        <tr style={{ borderBottom: "2px solid var(--bg3)" }}>
          {headers.map((h, i) => (
            <th key={i} style={{ padding: "4px 6px", fontWeight: 600,
                                  textAlign: i === 0 ? "left" : "right",
                                  color: h.color || "var(--text2)" }}>
              {h.label}
            </th>
          ))}
        </tr>
      ...
      <tbody>
        ...
        {totRows && (
          <>
            <tr><td colSpan={headers.length} style={{ padding: "2px 0" }} /></tr>
            {totRows}
          </>
        )}
      </tbody>
...
export function DashboardV2() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      dashboardV2.get(),
      dashboardV2.proprietari(),
      dashboardV2.stats(),
      dashboardV2.recenti(),
    ]).then(([d, dp, st, rc]) => {
      setDash(d); setDashProp(dp); setStats(st); setRecenti(rc);
  ...
  const kpis = [
    ...
    dashProp && {
      label: "Saldo Reale · Appartamenti",
    ...
    dashProp && (() => {
      ...
      const totale = list.reduce((s, p) => s + (p.incassato - p.pagato), 0);
      return {
        label: "Saldo Reale · Proprietari",
    ...
    stats && (() => {
      const nFatti = parseInt(stats.con_pdf || 0);
      return {
        label: "Fatti con PDF allegato",
        value: nFatti,
        icon: "ti-files", color: "#a855f7", bg: "#581c87",
        rows: [
          { label: "Totale fatti",    value: String(parseInt(stats.totale       || 0)), color: "#a855f7" },
          { label: "Da verificare",   value: String(parseInt(stats.da_verificare|| 0)), color: "#fbbf24" },
          { label: "Duplicati",       value: String(parseInt(stats.duplicati    || 0)), color: "#f87171" },
        ],
      };
    })(),
  ].filter(Boolean);
  ...
  if (loading) {
    return (
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center",
                    minHeight: 300, color: "var(--text2)", gap: 10 }}>
        <i className="ti ti-loader-2 ti-spin" style={{ fontSize: 24 }} />
        Calcolo dashboard in corso…
      </div>
    );
  }
  ...
  return (
    <div>
      <h2 style={{ fontSize: 22, fontWeight: 700, marginBottom: 4 }}>
        Dashboard
        <span style={{ fontSize: 11, fontWeight: 700, padding: "2px 8px", borderRadius: 12, marginLeft: 10,
      ...
      <p style={{ fontSize: 13, color: "var(--text2)", margin: "0 0 20px" }}>
        Dati calcolati su tutti gli immobili attivi dalla data del primo ruolo registrato ad oggi.
      </p>
      ...
      {/* ── Riepilogo Inquilini + Proprietari (Conguaglio) ── */}
        ...
        <div className="card">
          ...
          {dash ? (
            <Tabella
              headers={[
                { label: "Appartamento" },
                { label: "Spese",   color: "#a5b4fc" },
                { label: "Versato", color: "#4ade80" },
                { label: "Affitto", color: "#fbbf24" },
                { label: "Saldo" },
              ]}
              rows={dash.perAppartamento.map(a => (
          ...
          {dashProp ? (() => {
            ...
            return (
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
                <thead>
                  <tr style={{ borderBottom: "2px solid var(--bg3)" }}>
                    <th style={{ ...thP, textAlign: "left",  color: "var(--text2)" }}>App.</th>
                    <th style={{ ...thP, textAlign: "left",  color: "var(--text2)" }}>Proprietario</th>
                    <th style={{ ...thP, textAlign: "right", color: "#f87171" }} title="Dare teorico − Pagato reale">Δ Spese</th>
                    <th style={{ ...thP, textAlign: "right", color: "#4ade80" }} title="Avere teorico − Incassato reale">Δ Entrate</th>
                    <th style={{ ...thP, textAlign: "right", color: "#fbbf24" }}>Cong.</th>
                  </tr>
                </thead>
                <tbody>
                    ...
                    a.perProprietario.map((p, i) => {
                      ...
                      return (
                        <tr key={p.id + a.id} style={{ borderBottom: "1px solid var(--bg3)" }}>
                          <td style={{ padding: "6px 6px", fontSize: 10, color: "var(--text2)" }}>{i === 0 ? a.nome : ""}</td>
                          <td style={{ padding: "6px 6px", fontWeight: 500 }}>{p.nome}</td>
                          <td style={{ padding: "6px 6px", textAlign: "right", color: dSpese >= 0 ? "#f87171" : "#34d399" }}>
                            {dSpese >= 0 ? "+" : ""}{euro(dSpese)}
                          </td>
                          <td style={{ padding: "6px 6px", textAlign: "right", color: dEntrate >= 0 ? "#4ade80" : "#fb923c" }}>
                  ...
                  {totals.map(p => {
                    ...
                    return (
                      <tr key={"tot_" + p.nome} style={{ borderTop: "2px solid var(--border)", background: "var(--bg3)" }}>
                        ...
                        <td style={{ padding: "7px 6px", textAlign: "right", fontWeight: 700, color: dSpese >= 0 ? "#f87171" : "#34d399" }}>
                          {dSpese >= 0 ? "+" : ""}{euro(dSpese)}
                        </td>
                        <td style={{ padding: "7px 6px", textAlign: "right", fontWeight: 700, color: dEntrate >= 0 ? "#4ade80" : "#fb923c" }}>
                          {dEntrate >= 0 ? "+" : ""}{euro(dEntrate)}
                        </td>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
              <thead>
                <tr style={{ borderBottom: "2px solid var(--bg3)" }}>
                  ...
                  <th style={{ ...thP, textAlign: "left",  color: "var(--text2)" }}>Proprietario</th>
                  <th style={{ ...thP, textAlign: "right", color: "#f87171" }}>Spese Pagate</th>
                  <th style={{ ...thP, textAlign: "right", color: "#4ade80" }}>Entrate Incassate</th>
                  <th style={{ ...thP, textAlign: "right", color: "#34d399" }}>Saldo</th>
                </tr>
              </thead>
              <tbody>
                {dashProp.perAppartamento.flatMap(a =>
      ...
      {dash && (
        <div className="card" style={{ marginBottom: 16 }}>
          ...
          <Tabella
            ...
            rows={dash.perAppartamento.flatMap(a =>
            ...
            totRows={dash.perAppartamento.map(a => (
      ...
      {/* ── Fatti recenti con PDF ── */}
      {recenti.length > 0 && (
        <div className="card" style={{ marginBottom: 16 }}>
          <p style={{ fontWeight: 700, marginBottom: 12, fontSize: 15 }}>Fatti Recenti con PDF</p>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 8 }}>
            {recenti.slice(0, 8).map(d => (
              <div key={d.id} style={{ display: "flex", justifyContent: "space-between",
                ...
                <div style={{ flex: 1, minWidth: 0, marginRight: 8 }}>
                  <p style={{ fontWeight: 500, overflow: "hidden", textOverflow: "ellipsis",
                               whiteSpace: "nowrap", fontSize: 12, margin: 0 }}>
                    {d.nome_file || d.nome || "—"}
                  </p>
                  <p style={{ fontSize: 11, color: "var(--text2)", margin: 0 }}>
                    {d.immobile_nome || "—"} · {mesL(d.periodo_da || d.rif_da)}
                  </p>
                </div>
      ...
      {/* ── Affitti scoperti ── */}
      {dash && (() => {
        const conScoperti = dash.perAppartamento.filter(a => a.mesiScoperti?.length > 0);
        if (!conScoperti.length) return null;
        return (
          <div className="card" style={{ marginBottom: 16 }}>
            <p style={{ fontWeight: 700, marginBottom: 12, fontSize: 15 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 12 }}>
              {conScoperti.map(a => (
                <div key={a.id} style={{ background: "var(--bg2)", borderRadius: 8, padding: 12,
                  ...
                  {a.mesiScoperti.map(ms => (
                    <div key={ms.personaId} style={{ marginBottom: 8 }}>
                      ...
                      <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
                        {ms.mesi.map(m => (
                          <span key={m} style={{ background: "rgba(234,179,8,0.15)", color: "#eab308",
      ...
      {stats && (
        <div className="card">
          <p style={{ fontWeight: 700, marginBottom: 12, fontSize: 15 }}>Stato Fatti Economici</p>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12 }}>
            {[
              { label: "Elaborati",     v: stats.elaborati,     color: "var(--green)"  },
              { label: "Da verificare", v: stats.da_verificare, color: "var(--yellow)" },
              { label: "Errori",        v: stats.errori,        color: "var(--red)"    },
              { label: "Duplicati",     v: stats.duplicati,     color: "var(--purple)" },
            ].map(s => (
